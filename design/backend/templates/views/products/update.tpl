{capture name="mainbox"}
    ...
    {capture name="tabsbox"}
        ...
        <form id='form' action="{""|fn_url}" method="post" name="product_update_form" class="form-horizontal form-edit  cm-disable-empty-files {if ""|fn_check_form_permissions || ($runtime.company_id && $product_data.shared_product == "Y" && $product_data.company_id != $runtime.company_id)} cm-hide-inputs{/if}" enctype="multipart/form-data"> {* product update form *}
            ...
            <div class="product-manage" id="content_detailed"> {* content detailed *}
                ...
                <div id="acc_information" class="collapse in">
					...
					<div class="control-group">
                        <label for="product_seo_product_name" class="control-label">Seo name</label>
                        <div class="controls">
                            <input class="input-large" type="text" name="product_data[seo_product_name]" id="product_seo_product_name" size="55" value="{$product_data.seo_product_name}" />
                        </div>
                    </div>
					...
					<div class="control-group">
                        <label for="elm_indoor_price_price" class="control-label cm-required">Indoor {__("price")} ({$currencies.$primary_currency.symbol nofilter}):</label>
                        <div class="controls">
                            <input type="text" name="product_data[indoor_price]" id="elm_indoor_price_price" size="10" value="{$product_data.indoor_price|default:"0.00"|fn_format_price:$primary_currency:null:false}" class="input-long" />
                        </div>
                    </div>
					
					<div class="control-group">
                        <label for="elm_shipping_price" class="control-label">Shipping_price:</label>
                        <div class="controls">
                            <input type="text" name="product_data[shipping_price]" id="elm_shipping_price" size="10" value="{$product_data.shipping_price|default:"0.00"|fn_format_price:$primary_currency:null:false}" class="input-long" />
                        </div>
                    </div>
					<div class="control-group">
                        <label for="elm_show_discount" class="control-label">Show discount:</label>
                        <div class="controls">
							<input type="hidden" name="product_data[show_discount]" value="0"/>
                            <input type="checkbox" name="product_data[show_discount]" id="elm_show_discount" value="1" {if $product_data.show_discount}checked{/if}/>
                        </div>
                    </div>
				...
				{include file="common/subheader.tpl" title="UPC" target="#upc"}

                <div id="upc" class="collapse in">
					<div class="control-group">
                        <label for="elm_price_upc" class="control-label">UPC:</label>
                        <div class="controls">
                            <input type="text" name="product_data[upc]" id="elm_price_upc" size="100" value="{$product_data.upc}" class="input-long" />
                        </div>
                    </div>
					<div class="control-group">
                        <label for="elm_price_upc_lang" class="control-label">UPC lang:</label>
                        <div class="controls">
                            <input type="text" name="product_data[upc_lang]" id="elm_price_upc_lang" size="100" value="{$product_data.upc_lang}" class="input-long" />
                        </div>
                    </div>
				</div>
			...
			{** Product Instruction images section **}
            <div id="content_instruction_images" class="hidden clearfix"> {* content Instruction images *}
				{include file="common/subheader.tpl" title='Instruction Images'}
                {if $product_data.image_instruction_pairs}
                    <div class="cm-sortable sortable-box" data-ca-sortable-table="images_links" data-ca-sortable-id-name="pair_id" id="instruction_images">
                        {assign var="new_image_position" value="0"}
                        {foreach from=$product_data.image_instruction_pairs item=pair name="detailed_images"}
                            <div class="cm-row-item cm-sortable-id-{$pair.pair_id} cm-sortable-box">
                                <div class="cm-sortable-handle sortable-bar"><img src="{$images_dir}/icon_sort_bar.gif" width="26" height="25" border="0" title="{__("sort_images")}" alt="{__("sort")}" class="valign" /></div>
                                <div class="sortable-item">
                                    {include file="common/attach_images.tpl" image_name="product_instruction" image_object_type="product_instruction" image_key=$pair.pair_id image_type="A" image_pair=$pair icon_title=__("additional_thumbnail") detailed_title=__("additional_popup_larger_image") icon_text=__("text_additional_thumbnail") detailed_text=__("text_additional_detailed_image") delete_pair=true no_thumbnail=true}
                                </div>
                                <div class="clear"></div>
                            </div>
                            {if $new_image_position <= $pair.position}
                ...
                <div id="box_new_instruction_image">
                    <div class="clear cm-row-item">
                        <input type="hidden" name="product_add_instruction_image_data[0][position]" value="{$new_instruction_image_position}" class="cm-image-field" />
                        <div class="image-upload-wrap pull-left">{include file="common/attach_images.tpl" image_name="product_add_instruction" image_object_type="product_instruction" image_type="A" icon_title=__("additional_thumbnail") detailed_title=__("additional_popup_larger_image") icon_text=__("text_additional_thumbnail") detailed_text=__("text_additional_detailed_image") no_thumbnail=true}</div>
                        <div class="pull-right">{include file="buttons/multiple_buttons.tpl" item_id="new_instruction_image"}</div>
                    </div>
                </div>
			</div> {* /content Instruction images *}
            {** /Product Instruction images section **}
			
			{** Product feature images section **}
            <div id="content_feature_images" class="hidden clearfix"> {* content feature images *}
				{include file="common/subheader.tpl" title='Features Images'}
                {if $product_data.image_feature_pairs}
                    <div class="cm-sortable sortable-box" data-ca-sortable-table="images_links" data-ca-sortable-id-name="pair_id" id="feature_images">
                        {assign var="new_feature_image_position" value="0"}
                        {foreach from=$product_data.image_feature_pairs item=pair name="detailed_images"}
                            <div class="cm-row-item cm-sortable-id-{$pair.pair_id} cm-sortable-box">
                                <div class="cm-sortable-handle sortable-bar"><img src="{$images_dir}/icon_sort_bar.gif" width="26" height="25" border="0" title="{__("sort_images")}" alt="{__("sort")}" class="valign" /></div>
                                <div class="sortable-item">
                                    {include file="common/attach_images.tpl" image_name="product_feature" image_object_type="product_feature" image_key=$pair.pair_id image_type="A" image_pair=$pair icon_title=__("additional_thumbnail") detailed_title=__("additional_popup_larger_image") icon_text=__("text_additional_thumbnail") detailed_text=__("text_additional_detailed_image") delete_pair=true no_thumbnail=true}
                                </div>
                                <div class="clear"></div>
                            </div>
                            {if $new_feature_image_position <= $pair.position}
                                {assign var="new_feature_image_position" value=$pair.position}
                            {/if}
                        {/foreach}
                    </div>
                {/if}

                <div id="box_new_feature_image">
                    <div class="clear cm-row-item">
                        <input type="hidden" name="product_add_feature_image_data[0][position]" value="{$new_feature_image_position}" class="cm-image-field" />
                        <div class="image-upload-wrap pull-left">{include file="common/attach_images.tpl" image_name="product_add_feature" image_object_type="product_feature" image_type="A" icon_title=__("additional_thumbnail") detailed_title=__("additional_popup_larger_image") icon_text=__("text_additional_thumbnail") detailed_text=__("text_additional_detailed_image") no_thumbnail=true}</div>
                        <div class="pull-right">{include file="buttons/multiple_buttons.tpl" item_id="new_feature_image"}</div>
                    </div>
                </div>
			</div> {* /content feature images *}
            {** /Product feature images section **}
			
			{** Product links section **}
            <div id="content_sauna_type_links" class="hidden clearfix"> {* content links *}
				{include file="common/subheader.tpl" title='Sauna type links'}
				<div class="control-group">
					<label for="elm_product_indoor_link" class="control-label">Product indoor link:</label>
					<div class="controls">
						<input type="text" name="product_data[product_indoor_link]" id="elm_product_indoor_link" value="{$product_data.product_indoor_link}" class="input-large" />
					</div>
				</div>
				<div class="control-group">
					<label for="elm_product_sloope_roof_link" class="control-label">Product Sloope Roof Link:</label>
					<div class="controls">
						<input type="text" name="product_data[product_sloope_roof_link]" id="elm_product_sloope_roof_link" value="{$product_data.product_sloope_roof_link}" class="input-large" />
					</div>
				</div>
				<div class="control-group">
					<label for="elm_product_peak_roof_link" class="control-label">Product Peak Roof Link:</label>
					<div class="controls">
						<input type="text" name="product_data[product_peak_roof_link]" id="elm_product_peak_roof_link" value="{$product_data.product_peak_roof_link}" class="input-large" />
					</div>
				</div>
			</div> {* /content links *}
            {** /Product links section **}
                ...
                {if $id}
                    {capture name="tools_list"}
                        {hook name="products:update_tools_list"}
							...
							<li>{btn type="list" text="Save PDF" href="products.print_product?product_id=`$id`"}</li>
                            {if $allow_save}
