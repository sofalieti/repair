<select id="elm_{$elm_id}" name="page_data[form][elements_data][{$num}][element_type]" onchange="fn_check_element_type(this.value, this.id, '{$selectable_elements}');">
	...
	<option value="{$smarty.const.FORM_OPEN_GROUP}" {if $element_type == $smarty.const.FORM_OPEN_GROUP}selected="selected"{/if}>Open group</option>
	<option value="{$smarty.const.FORM_CLOSE_GROUP}" {if $element_type == $smarty.const.FORM_CLOSE_GROUP}selected="selected"{/if}>Close group</option>
    </optgroup>
	...
	<option value="{$smarty.const.FORM_MULTI_IMAGES}" {if $element_type == $smarty.const.FORM_MULTI_IMAGES}selected="selected"{/if}>Multi Images</option>
    {/hook}
    </optgroup>
</select>