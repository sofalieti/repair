	...
	<div class="mobile-row navbar-expand-md">
		<ul class="ty-menu__items cm-responsive-menu navbar-nav nav-fill w-100">
			...
			{foreach from=$items item="item1" name="item1"}
				...
				<li class="nav-item  css_{$item1.param_id}_menu_top ty-menu__item{if !$item1.$childs} ty-menu__item-nodrop{else} cm-menu-item-responsive{/if}{if $item1.active || $item1|fn_check_is_active_menu_item:$block.type} ty-menu__item-active{/if}{if $item1.class} {$item1.class}{/if}">
						{if $item1.$childs}
							<a class="css_{$item1.param_id}_menu_bot ty-menu__item-toggle visible-phone cm-responsive-menu-toggle">
								<i class="ty-menu__icon-open ty-icon-down-open"></i>
								<i class="ty-menu__icon-hide ty-icon-up-open"></i>
							</a>
						{/if}
						<a {if $item1_url} href="{$item1_url}"{/if} class="ty-menu__item-link css_{$item1.param_id}_menu_bot">
						   {if $item1.$name == 'HOME_ICON'}{*<i class="fas fa-home d-none d-md-inline"></i>*}<img src="/images/companies/1/guruhome2.png" class="d-none d-md-inline"/>&nbsp;<span class="d-sm-inline d-md-none">Home</span>{else}{$item1.$name}{/if}
						</a>
					{if $item1.$childs}
						...
						{* Only two levels. Vertical output *}
							<div class="ty-menu__submenu ">
								<ul class="ty-menu__submenu-items ty-menu__submenu-items-simple cm-responsive-menu-submenu submenu{$item1.param_id}">
