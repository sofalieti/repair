...
{hook name="products:product_name"}
    ...
    {elseif $show_trunc_name}
        {if $hide_links}<strong>{else}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title" title="{$product.product|strip_tags}" {live_edit name="product:product:{$product.product_id}" phrase=$product.product}>{/if}{$product.product|truncate:80:"...":true nofilter}{if $hide_links}</strong>{else}</a>{/if}
    {/if}
{/hook}
{/capture}
{if $no_capture}
...
{********************** My ******************}
{capture name="my_my_`$obj_id`"}
    {if $show_price_values && $show_old_price}
        <span class="cm-reload-{$obj_prefix}{$obj_id}" id="my_my_update_{$obj_prefix}{$obj_id}">
            {hook name="products:my_my"}
            {if $product.discount}
                <span class="ty-list-price ty-nowrap" id="my_my_price_{$obj_prefix}{$obj_id}">{$product.my  nofilter}</span> 
            ...
            {/hook}
        <!--my_my_update_{$obj_prefix}{$obj_id}--></span>
    {/if}
{/capture}
{if $no_capture}
    {assign var="capture_name" value="my_my_`$obj_id`"}
    {$smarty.capture.$capture_name nofilter}
{/if}
...
{********************** Price *********************}
{capture name="price_allsaunas_`$obj_id`"}
        ...
        {if $show_price_values}
            ...
            {hook name="products:prices_block"}
                ...
                {elseif $product.zero_price_action == "R"}
                   {*<a href="/contact-us-for-price.html"><span class="ty-no-price">{__("contact_us_for_price")}</span></a>*}
                    <a class="cm-dialog-opener cm-dialog-auto-size btn btn-primary" data-ca-view-id="contact_us_for_a_price_{$product.product_id}" data-ca-target-id="contact_us_for_a_price_{$product.product_id}" href="/index.php?dispatch=products.contact_us_for_a_price&product_id={$product.product_id}&product={$product.product}" data-ca-dialog-title="Contact us for a price" rel="nofollow"><span class="ty-no-price">{__("contact_us_for_price")}</span></a>
                    {assign var="show_qty" value=false}
                {/if}
            {/hook}
            {/if}
...
{/capture}
{if $no_capture}
    {assign var="capture_name" value="price_allsaunas_`$obj_id`"}
    {$smarty.capture.$capture_name nofilter}
{/if}
    ...
    <span class="cm-reload-{$obj_prefix}{$obj_id} ty-price-update" id="price_update_{$obj_prefix}{$obj_id}">
        ...
        {if $show_price_values}
            ...
            {hook name="products:prices_block"}
                ...
                {elseif $product.zero_price_action == "R"}
                   {*<a href="/contact-us-for-price.html"><span class="ty-no-price">{__("contact_us_for_price")}</span></a>*}
                    <a class="cm-dialog-opener cm-dialog-auto-size btn btn-primary" data-ca-view-id="contact_us_for_a_price_{$product.product_id}" data-ca-target-id="contact_us_for_a_price_{$product.product_id}" href="/index.php?dispatch=products.contact_us_for_a_price&product_id={$product.product_id}&product={$product.product}" data-ca-dialog-title="Contact us for a price" rel="nofollow">{__("contact_us_for_price")}</a>
                    {assign var="show_qty" value=false}
                {/if}
            {/hook}
            {/if}
...
{capture name="product_picked_options_`$obj_id`"}
    {if $show_product_options}
    <div class="cm-reload-{$obj_prefix}{$obj_id}" id="product_options_update_{$obj_prefix}{$obj_id}">
        <input type="hidden" name="appearance[show_product_options]" value="{$show_product_options}" />
        {hook name="products:product_option_content"}
...
{/capture}
{if $no_capture}
    {assign var="capture_name" value="product_picked_options_`$obj_id`"}
    {$smarty.capture.$capture_name nofilter}
{/if}
