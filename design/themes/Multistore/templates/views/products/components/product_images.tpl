{assign var="th_size" value=$thumbnails_size|default:45}
...
{else}
	{assign var=new_pair value=$product.image_pairs|current}
	{if $new_pair.icon || $new_pair.detailed}
		{assign var="image_pair_var" value=$new_pair}
	{/if}
{/if}
...
{foreach from=$product.image_pairs item="image_pair"}
    {if ($image_pair && !isset($new_pair)) || (isset($new_pair) && $new_pair.pair_id != $image_pair.pair_id)}
...
{if $product.image_pairs}
    ...
    {else}
        <div class="ty-product-thumbnails ty-center cm-image-gallery" id="images_preview_{$preview_id}" style="width: {$image_width}px;">
            {strip}
                {if $image_pair_var}
				{if !empty($image_pair_var.detailed.alt)}
				<div class="product-thumb-alt-title">{$image_pair_var.detailed.alt}</div>
				{/if}
                    ...
                    {foreach from=$product.image_pairs item="image_pair"}
                        {if $image_pair}
                                ...
                                {else}
                                    {assign var="img_id" value=$image_pair.image_id}
                                {/if}
								{if !empty($image_pair.detailed.alt)}
								<div class="product-thumb-alt-title">{$image_pair.detailed.alt}</div>
								{/if}
