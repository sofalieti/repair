{if $page.page_type == $smarty.const.PAGE_TYPE_FORM}
    {assign var="form_label_as_placeholder" value=$smarty.const.FORM_LABEL_AS_PLACEHOLDER}
    ...
    {else}
        ...
        {if $page.form_left}
            ...
            {if $page.description}
                <div class="ty-form-builder__description {if $page.form_left}col-md-8{/if}">{$page.description nofilter}</div>
            {/if}

            <div class="ty-form-builder ty-form-builder-{$page.page_id} {if $page.form_left}col-md-8{/if}">
                <form action="{""|fn_url}" method="post" name="forms_form" enctype="multipart/form-data">
                    ...
                    {foreach from=$page.form.elements key="element_id" item="element" name="form_lements"}
                        ...
                        {elseif $element.element_type == $smarty.const.FORM_HEADER}
                            ...
                            {include file="common/subheader.tpl" title=$element.description class="ty-subheader form_group_`$element.element_id`"}

                        {elseif $element.element_type == $smarty.const.FORM_OPEN_GROUP}

                            <div class="form-elements-group {$element.css_class}">

                            {elseif $element.element_type == $smarty.const.FORM_CLOSE_GROUP}

                            </div>

                        {elseif $element.element_type != $smarty.const.FORM_IP_ADDRESS && $element.element_type != $smarty.const.FORM_REFERER}
                            <div class="form-group ty-control-group {$element.css_class}" id="form_group_{$element.element_id}">
                                {if $element.element_type == $smarty.const.FORM_RADIO || $element.element_type == $smarty.const.FORM_CHECKBOX || $element.element_type == $smarty.const.FORM_MULTIPLE_SB || $element.element_type == $smarty.const.FORM_MULTIPLE_CB || $element.element_type == $smarty.const.FORM_FILE}
                                    <label for="{if $element.element_type == $smarty.const.FORM_FILE}type_{"fb_files[`$element.element_id`]"|md5}{else}elm_{$element.element_id}{/if}" class="ty-control-group__title {if $element.required == "Y"}cm-required{/if}{if $element.element_type == $smarty.const.FORM_EMAIL} cm-email{/if}{if $element.element_type == $smarty.const.FORM_PHONE} cm-phone{/if} {if $element.element_type == $smarty.const.FORM_MULTIPLE_CB}cm-multiple-checkboxes{/if} cm-label-type-{$element.element_type}">{$element.description}</label>
                                {elseif $page.form.general.$form_label_as_placeholder ne "Y"}
                                    <label for="{if $element.element_type == $smarty.const.FORM_FILE}type_{"fb_files[`$element.element_id`]"|md5}{else}elm_{$element.element_id}{/if}" class="ty-control-group__title {if $element.required == "Y"}cm-required{/if}{if $element.element_type == $smarty.const.FORM_EMAIL} cm-email{/if}{if $element.element_type == $smarty.const.FORM_PHONE} cm-phone{/if} {if $element.element_type == $smarty.const.FORM_MULTIPLE_CB}cm-multiple-checkboxes{/if} cm-label-type-{$element.element_type}">{$element.description}</label>
                                {else}
                                    <label for="{if $element.element_type == $smarty.const.FORM_FILE}type_{"fb_files[`$element.element_id`]"|md5}{else}elm_{$element.element_id}{/if}" class="hidden ty-control-group__title {if $element.required == "Y"}cm-required{/if}{if $element.element_type == $smarty.const.FORM_EMAIL} cm-email{/if}{if $element.element_type == $smarty.const.FORM_PHONE} cm-phone{/if} {if $element.element_type == $smarty.const.FORM_MULTIPLE_CB}cm-multiple-checkboxes{/if} cm-label-type-{$element.element_type}">{$element.description}</label>
                                {/if}


                                {if $element.element_type == $smarty.const.FORM_SELECT}
                                    <select id="elm_{$element.element_id}" class="form-control ty-form-builder__select" name="form_values[{$element.element_id}]" {if $page.form.general.$form_label_as_placeholder eq "Y"}placeholder="{$element.description}"{/if}>
                                        <option label="" value="">-  {if $page.form.general.$form_label_as_placeholder eq "Y"}{$element.description}{else}{__("select")}{/if} -</option>
                                ...
                                {elseif $element.element_type == $smarty.const.FORM_MULTIPLE_SB}
                                    <select class="form-control  ty-form-builder__multiple-select" id="elm_{$element.element_id}" name="form_values[{$element.element_id}][]" multiple="multiple" >
                                        {foreach from=$element.variants item=var}
                                ...
                                {elseif $element.element_type == $smarty.const.FORM_INPUT}
                                    <input id="elm_{$element.element_id}" class="form-control  ty-form-builder__input-text ty-input-text {if $element.position == 1}cm-focus {/if}" size="50" type="text" name="form_values[{$element.element_id}]" value="{$form_values.$element_id}"  {if $page.form.general.$form_label_as_placeholder eq "Y"}placeholder="{$element.description}"{/if}/>

                                {elseif $element.element_type == $smarty.const.FORM_TEXTAREA}
                                    <textarea id="elm_{$element.element_id}" class="form-control  ty-form-builder__textarea" name="form_values[{$element.element_id}]" cols="67" rows="10" {if $page.form.general.$form_label_as_placeholder eq "Y"}placeholder="{$element.description}"{/if}>{$form_values.$element_id}</textarea>

                                {elseif $element.element_type == $smarty.const.FORM_DATE}
                                    {include file="common/calendar.tpl" placeholder={$element.description} date_name="form_values[`$element.element_id`]" date_id="elm_`$element.element_id`" date_val=$form_values.$element_id}
                                ...
                                {elseif $element.element_type == $smarty.const.FORM_EMAIL || $element.element_type == $smarty.const.FORM_NUMBER || $element.element_type == $smarty.const.FORM_PHONE}
                                    ...
                                    {/if}
                                    <input id="elm_{$element.element_id}" class="form-control  ty-input-text {if $element.position == 1}cm-focus {/if}" size="50" type="text" name="form_values[{$element.element_id}]" value="{$form_values.$element_id}"  {if $page.form.general.$form_label_as_placeholder eq "Y"}placeholder="{$element.description}"{/if} />
                                ...
                                {elseif $element.element_type == $smarty.const.FORM_COUNTRIES}
                                    ...
                                    <select id="elm_{$element.element_id}" name="form_values[{$element.element_id}]" class="form-control  ty-form-builder__country cm-country cm-location-billing">
                                ...
                                {elseif $element.element_type == $smarty.const.FORM_STATES}
                                    ...
                                    <select class="form-control ty-form-builder__state cm-state cm-location-billing" id="elm_{$element.element_id}" name="form_values[{$element.element_id}]">
                                        <option label="" value="">- {__("select_state")} -</option>
                                    </select>
                                    <input type="text" class="form-control  cm-state cm-location-billing ty-input-text hidden" id="elm_{$element.element_id}_d" name="form_values[{$element.element_id}]" size="32" maxlength="64" value="{$_state}" disabled="disabled" />
                                ...
                                {elseif $element.element_type == $smarty.const.FORM_MULTI_IMAGES}
                                    <input type="file" name="file_fb_images[{$element.element_id}][]" multiple id="elm_{$element.element_id}"/>
                                {/if}
                    ...
                    <div class="form-elements-group ty-form-builder__buttons buttons-container text-right">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                {include file="buttons/button.tpl" but_role="submit" but_text=__("submit") but_meta="btn-primary btn-block" but_name="dispatch[pages.send_form]"}
                            </div>
