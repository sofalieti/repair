...
//if(!isset($_SESSION['sauna_type'])) $_SESSION['sauna_type'] = 'outdoor';
...
function fn_get_rewrite_rules()
{
    ...
    ///////       
    $rewrite_rules['!^' . $prefix . '\/(.*)?\/(.*\/)?([^\/]+)\.(' . $extension . ')$!'] = 'object_name=$matches[4]&sl=$matches[1]&extension=$matches[5]&brand=$matches[2]';
...
function fn_seo_get_route(&$req, &$result, &$area, &$is_allowed_url)
{
    if (($area == 'C') && !$is_allowed_url) {
        ...
        /*if(preg_match('/^\/(indoor\/{1}|outdoor\/{1})/i', $uri, $uri_result)){
                if(isset($uri_result[1])){
                        $uri = str_replace($uri_result[1], '', $uri);
                        //$_SESSION['sauna_type'] = str_replace("/", "", $uri_result[1]);
                        $_SESSION['sauna_type'] = 'outdoor';
                        $_SESSION['sauna_type_image'] = str_replace("/", "", $uri_result[1]);
                }
        }*/
    ...
    if (empty($is_allowed_url)) {
		...
		$uri = str_replace('/outdoor', '', $uri);
		$uri = str_replace('/indoor', '', $uri);
...
function fn_seo_url_post(&$url, &$area, &$original_url, &$prefix, &$company_id_in_url, &$lang_code)
{
    ...
    //////
    if(isset($parsed_query['brand_id'])){
        $brand = fn_brands_get_brand_data($parsed_query['brand_id']);
        unset($parsed_query['brand_id']);
        $link_parts['brand'] = $brand['seo_name'].'/';
    }
