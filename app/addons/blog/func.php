...
function fn_blog_get_posts_by_show_on_main_page(){
	$posts = db_get_array("SELECT ?:pages.*, ?:page_descriptions.* FROM ?:pages LEFT JOIN ?:page_descriptions ON(?:page_descriptions.page_id = ?:pages.page_id) WHERE ?:pages.page_type = ?s AND ?:pages.show_on_main_page = ?s ORDER BY ?:pages.timestamp DESC", "B", "Y");
	foreach($posts as $key => $post){
		$posts[$key]['main_pair'] = fn_get_image_pairs($post['page_id'], 'page_image', 'M', true, true, $lang_code);
	}
	return $posts;
}