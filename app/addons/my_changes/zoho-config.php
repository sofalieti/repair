<?php
$client_id = '1000.BTF1EEFQHB7C01002COLXPRW46NORB';
$client_secret = 'a58d9e41f7f3810ad72b865242ce9946ab4e092e29';
$redirect_uri = 'https://outdoorinfraredsauna.com/admin.php?dispatch=zoho.get_orders';
$refresh_token = $oauth_data_json -> OAUTH_REFRESH_TOKEN;
$oauth_scope = 'Desk.tickets.READ,Desk.search.READ,Desk.tickets.UPDATE';

$zoho_auth = db_get_row('SELECT * FROM ?:zoho_authentications WHERE type = ?s', 'get_orders');
	...
	if(time() > $zoho_auth['created_at']){
		...
		if($info['http_code']==200){
			db_query('UPDATE ?:zoho_authentications SET created_at = ?i, access_token = ?s WHERE type = ?s', 
				(time()+3600-10), $auth_response['access_token'], 'get_orders');	
		}
		else{
			echo " Error while getting OAuth Token ::::: ";print_r($auth_response);
		}    
