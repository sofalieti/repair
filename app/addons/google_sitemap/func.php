...
function fn_google_sitemap_generate_link($object, $value, $languages, $extra = array())
{
    switch ($object) {
        case 'product':
            ...
            break;
        case 'category':
            $link = 'categories.view?category_id=' . $value . (isset($extra['brand_id']) ? "&brand_id={$extra['brand_id']}" : "");
...
HEAD;
    ...
    if (!empty($get_categories)) {
        $categories = db_get_array("SELECT category_id, parent_id FROM ?:categories WHERE FIND_IN_SET(?i, usergroup_ids) AND status = 'A' ?p", USERGROUP_ALL, fn_get_google_sitemap_company_condition('?:categories.company_id'));

        fn_set_progress('step_scale', count($categories));

        //Add the all active categories
        foreach ($categories as $category) {
            if($category['parent_id'] == 389){
                foreach(db_get_array("SELECT * FROM ?:brands ORDER BY name") as $brand){

                    $links = fn_google_sitemap_generate_link('category', $category['category_id'], $languages, array('brand_id' => $brand['brand_id']));
                ...
                $links = fn_google_sitemap_generate_link('category', $category['category_id'], $languages);
