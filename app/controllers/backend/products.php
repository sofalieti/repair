...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'update') {
        if (!empty($_REQUEST['product_data']['product'])) {
            ...
            fn_attach_image_pairs('product_instruction', 'product_instruction', $product_id, DESCR_SL);

            // Adding instruction images
            fn_attach_image_pairs('product_add_instruction', 'product_instruction', $product_id, DESCR_SL);
			
			//Update instruction images
            fn_attach_image_pairs('product_feature', 'product_feature', $product_id, DESCR_SL);

            // Adding instruction images
            fn_attach_image_pairs('product_add_feature', 'product_feature', $product_id, DESCR_SL);
	...
	if ($mode == 'save_pdf_products') {
		if(isset($_REQUEST['product_ids'])){
			$product_ids = $_REQUEST['product_ids'];
			foreach($product_ids as $product_id){
				fn_print_product($product_id, DESCR_SL, true);
			}
			fn_set_notification('N', __('notice'), "PDF saved");
		}
		return array(CONTROLLER_STATUS_REDIRECT, 'products.manage');
	}
...
//
	...
	$product_data['image_instruction_pairs'] = fn_get_image_pairs($_REQUEST['product_id'], 'product_instruction', 'A', true, true, DESCR_SL);
	$product_data['image_feature_pairs'] = fn_get_image_pairs($_REQUEST['product_id'], 'product_feature', 'A', true, true, DESCR_SL);
    ...
    $tabs = array (
        ...
        'sauna_type_links' => array (
            'title' => 'Sauna Type Links',
            'js' => true
        ),
        'seo' => array(
            'title' => __('seo'),
...
//
    ...
    exit;
} elseif ($mode == 'update_code_by_id') {
	foreach(db_get_fields('SELECT product_id FROM ?:products') as $product_id){
		db_query('UPDATE ?:products SET product_code = ?s WHERE product_id = ?i',$product_id,$product_id);
	}
	die('END');
}elseif ($mode == 'print_product') {
    if (!empty($_REQUEST['product_id'])) {
        fn_print_product($_REQUEST['product_id']);
		return array(CONTROLLER_STATUS_REDIRECT, 'products.update?product_id=' . $_REQUEST['product_id']);
    }
    exit;
}
...
function fn_print_product($product_id, $lang_code = DESCR_SL, $only_save = false)
{
	set_time_limit(999);
    $view = Tygh::$app['view'];
    $html_ourdoor = array();
	...
	Tygh::$app['view']->assign('sauna_type', 'outdoor');
	$html_ourdoor[] = $view->displayMail('products/print_product.tpl', false, 'A', 1, $lang_code);
	
	Tygh::$app['view']->assign('sauna_type', 'indoor');
	$html_indoor[] = $view->displayMail('products/print_product.tpl', false, 'A', 1, $lang_code);
	
	//echo $html[0];
	//exit;
	
	Pdf::render($html_ourdoor, $_SERVER['DOCUMENT_ROOT'].'/PDF/PDF_'.$product['product_id'].".pdf", true);
	Pdf::render($html_indoor, $_SERVER['DOCUMENT_ROOT'].'/PDF/PDF_'.$product['product_id']."_indoor.pdf", true);
	
	if(!$only_save){
		//Pdf::render($html_ourdoor, 'Product Outdoor #'.$product['product_id']);
		//Pdf::render($html_indoor, 'Product Indoor #'.$product['product_id']);
		fn_set_notification('N', __('notice'), "PDF saved <a href='/PDF/PDF_{$product['product_id']}.pdf' target='_blank'>PDF outdoor</a>, <a href='/PDF/PDF_{$product['product_id']}_indoor.pdf' target='_blank'>PDF indoor</a>");
	}		
    return true;
}
