...
function fn_send_form($page_id, $form_values)
{
    ...
    if (!empty($form_values)) {
		...
		$url = "https://support.infraredsaunaparts.com/support/WebToCase";
		...
		if(strtolower($v['description']) == 'first name'){
			$zoho_data['First Name'] = $form_values[$k];
		}
		elseif(strtolower($v['description']) == 'last name'){
			$zoho_data['Contact Name'] = $form_values[$k];
		}
		elseif(strtolower($v['description']) == 'e-mail'){
			$zoho_data['Email'] = $form_values[$k];
		}
		elseif(strtolower($v['description']) == 'phone'){
			$zoho_data['Phone'] = $form_values[$k];
		}else{
			$zoho_data['Description'] .= $v['description'].": ".$form_values[$k]."<br/>";
		}
            }
