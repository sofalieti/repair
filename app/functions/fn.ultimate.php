...
function fn_init_store_params_by_host(&$request, $area = AREA)
{
	...
	if(AREA == 'C'){////
		if(!$companies){
			$o_domain_default = db_get_row('SELECT * FROM ?:domains WHERE `default` = ?i', 1);
			if($o_domain_default){
				$companies = db_get_array("SELECT company_id, $field FROM ?:companies WHERE $field RLIKE ?l", '^(www[0-9]*.)?' . $o_domain_default['name']);
				$o_domain = $_SERVER['HTTP_HOST'];
			}
		}
	}
    ...
    if (!empty($request['switch_company_id']) && $request['switch_company_id'] != 'all' && !isset($request['skip_config_changing'])) { // theme for company with id = 0 cannot be loaded.
		...
		if(isset($o_domain)) $company_data[$field] = $o_domain;////
