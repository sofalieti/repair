...
function fn_get_product_data($product_id, &$auth, $lang_code = CART_LANGUAGE, $field_list = '', $get_add_pairs = true, $get_main_pair = true, $get_taxes = true, $get_qty_discounts = false, $preview = false, $features = true, $skip_company_condition = false, $feature_variants_selected_only = false)
{
    ...
    if (!empty($product_id)) {
        ...
        // Get main image pair
		//$sauna_type = $_SESSION['sauna_type'];
		$sauna_type = 'outdoor';
		if(Registry::get('runtime')){
			if(((Registry::get('runtime')['controller'] == 'products' && Registry::get('runtime')['mode'] == 'view') ||
				(Registry::get('runtime')['controller'] == 'categories' && Registry::get('runtime')['mode'] == 'all_models')) && $_SESSION['sauna_type_image'] == 'indoor'){
				$sauna_type = 'indoor';
			}
		}
        if ($get_main_pair == true) {
			...
			if(AREA == "C"){
				if(($sauna_type == 'indoor' && $product_data['main_pair']['detailed']['alt'] != 'indoor') || ($sauna_type == 'outdoor' && $product_data['main_pair']['detailed']['alt'] == 'indoor')){
					$product_data['main_pair'] = array();
				}
			}
        }
        ...
        if ($get_add_pairs == true) {
			...
			if(AREA == "C"){
				foreach($product_data['image_pairs'] as $key => $pair){
					if(($sauna_type == 'indoor' && $pair['detailed']['alt'] != 'indoor') || ($sauna_type == 'outdoor' && $pair['detailed']['alt'] == 'indoor')){
						unset($product_data['image_pairs'][$key]);
					}
				}
			}
        }
	...
	if(AREA != 'A'){
		if(!isset($_SESSION['price_products'])) 
			$product_data['price'] = 0;
		elseif(!in_array($product_data['product_id'],$_SESSION['price_products']))
			$product_data['price'] = 0;
	}
...
function fn_gather_additional_products_data(&$products, $params)
{
    ...
    if ($params['get_icon'] || $params['get_detailed']) {
		...
		if(AREA == "C"){
			foreach($products_images as $key => $pairs){
				foreach($pairs as $key2 => $image){
					if(($sauna_type == 'indoor' && $image['detailed']['alt'] != 'indoor') || ($sauna_type == 'outdoor' && $image['detailed']['alt'] == 'indoor')){
						unset($products_images[$key][$key2]);
					}
				}
			}
		}
    ...
    if ($params['get_additional']) {
		...
		if(AREA == "C"){
			foreach($additional_images as $key => $pairs){
				foreach($pairs as $key2 => $image){
					if(($sauna_type == 'indoor' && $image['detailed']['alt'] != 'indoor') || ($sauna_type == 'outdoor' && $image['detailed']['alt'] == 'indoor')){
						unset($additional_images[$key][$key2]);
					}
				}
			}
		}
...
//$product['my']="my_test2233";/
			...
			//$product[$my] = $product['base_modifier'];
    ...
    $mystring2 =  fn_apply_options_modifiers_string($selected_options, $product['original_price'], 'P', array(), array('product_data' => $product),$product_options);

 
			$product['my'] =  $mystring2;
			//$product['my'] = fn_get_product_option_data(175, 10,"EN");

		
                if (empty($product['original_price'])) {
                    $product['original_price'] = $old_price;
                }
$product['WebPrice'] =  floatval($product['original_price']);
 
$product['MSRP'] =  floatval($product['list_price']);
	...
	if ($params['get_features'] == true && !isset($product['product_features'])) {
		if(isset($params['CP'])){
			$product['product_features'] = fn_get_product_features_list($product['product_id'], 'CP');
		}else{
			$product['product_features'] = fn_get_product_features_list($product['product_id'],$params['features_display_on']);
		}
	}
...
function fn_get_category_data($category_id = 0, $lang_code = CART_LANGUAGE, $field_list = '', $get_main_pair = true, $skip_company_condition = false, $preview = false)
{
    ...
    if (!empty($category_data)) {
		if(AREA == 'C'){
			$category_data['category'] = str_replace('!sauna_type!', $_SESSION['sauna_type'], $category_data['category']);
			$category_data['category'] = str_replace('!sierra_line!', ($_SESSION['sauna_type'] == 'outdoor' ? 'Sierra' : 'Golden'), $category_data['category']);
			$category_data['category'] = str_replace('!rustic_line!', ($_SESSION['sauna_type'] == 'outdoor' ? 'Rustic' : 'Vitality'), $category_data['category']);
		}
...
function fn_get_product_options($product_ids, $lang_code = CART_LANGUAGE, $only_selectable = false, $inventory = false, $only_avail = false, $skip_global = false)
{
    ...
    $v_fields = "a.variant_id,a.pretext, a.option_id, a.position, a.modifier, a.modifier_type, a.weight_modifier, a.weight_modifier_type, $extra_variant_fields b.variant_name";
 ...
 */
 ...
 function fn_apply_options_modifiers_string($product_options, $base_value, $type, $orig_options = array(), $extra = array(),$my_options)
{
    static $option_types_cache = array();
    static $option_modifiers_cache = array();
	$x = 0;
$mystring="";
    $fields = ($type == 'P') ? "a.pretext,a.modifier, a.modifier_type" : "a.pretext,a.weight_modifier as modifier, a.weight_modifier_type as modifier_type";
    ...
    if (!empty($product_options)) {
        ...
        if (empty($orig_options)) {
            ...
            if (!isset($option_types_cache[$_key])) {
				...
				$option_names = db_get_hash_single_array("SELECT option_name, option_id FROM ?:product_options_descriptions WHERE option_id IN (?n)", array('option_id', 'option_name'), array_keys($product_options));
            ...
            if (!empty($_mod)) {
                if ($_mod['modifier_type'] == 'A') {
                    ...
                    if ($_mod['modifier']{0} == '-') {
						...
						$old_value = -($old_value - $base_value);
				
						if ($old_value!=0){
						 $mystring  = $mystring . "<div class=\"opt\"><div class=\"opt_styles\"><div class=\"optleft\"><span class=\"opt1\">" . $option_names[$option_id] . "</span><br> <span class=\"opt2\"><span class=\"OptName\">MSRP:</span> <strike>$" . sprintf("%1\$.2f",$_mod['pretext']) ."</strike></span>
<span class=\"opt3\"><br><span class=\"OptName\">Your savings:</span> $" . sprintf("%1\$.2f",($_mod['pretext']-$_mod['modifier'])) ."</span>
</div><div class=\"optright\"><span class=\"opt4\"><br><span class=\"OptName\">Final Sale Price:</span>  <span class=\"finaloption\">+ $" . sprintf("%1\$.2f",($_mod['modifier'])) ."</span></span></div>
</div></div>";
                    ...
                    if ($_mod['modifier']{0} == '-') {
                        ...
                        //$base_value = $base_value + ((floatval($_mod['modifier']) * $orig_value)/100);
							if ($_mod['modifier']!=0){
								...
								$price = db_get_field('SELECT price FROM ?:product_prices WHERE product_id = ?i AND lower_limit = ?i',$product_id,1);
								$p_price = floatval($price * round($_mod['modifier'])/100);
								$mystring  = $mystring . "<div class=\"opt\"><span class=\"opt1\">" . $option_names[$option_id] . "</span> <span class=\"opt2\">+".round($_mod['modifier'])."% ".$base_value."</span><span class=\"opt2\">= $".sprintf("%1\$.2f",$p_price)."</span></div>";
								$x++;
							}
                    }
                }
...
//$mystring = $mystring." kurdushka ".$base_value;
//   $mystring = "a1 is: '".implode("','",$option_names)."'<br>";
if ($x>0)
{
	
$mystring = "<div class=\"pricemod\"><span class=\"opt0\">Options Price Modificators</span></br>".$mystring."</div>";
...
function fn_get_selected_product_options_info($selected_options, $lang_code = CART_LANGUAGE)
{
    ...
    foreach ($selected_options as $option_id => $variant_id) {
        ...
        if (strpos('SRC', $_opts['option_type']) !== false) {
            $_vars = db_get_row(
                "SELECT a.pretext,a.modifier, a.modifier_type, a.position, b.variant_name FROM ?:product_option_variants as a " .
...
function fn_get_products($params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
    ...
    if(isset($params['letter']) && !empty($params['letter'])){
        $condition .= db_quote(' AND descr1.product LIKE "'.$params['letter'].'%"');
    }
    if(isset($params['letters']) && !empty($params['letters'])){
        $condition .= db_quote(' AND descr1.product REGEXP "^('. join("|", $params['letters']).'){1}"');
    }
	...
	if(AREA != 'A'){
		foreach($products as $k => $product){
			if(!isset($_SESSION['price_products'])) 
				$products[$k]['price'] = 0;
			elseif(!in_array($products[$k]['product_id'],$_SESSION['price_products']))
				$products[$k]['price'] = 0;
		}
	}
