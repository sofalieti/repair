   ...
   /**
    ...
    if (!empty($pages)) {
        foreach ($pages as $k => $v) {
			...
			$pages[$k]['main_pair'] = fn_get_image_pairs($v['page_id'], 'page_image', 'M', true, true, $lang_code);
        }
...
function fn_update_page($page_data, $page_id = 0, $lang_code = CART_LANGUAGE)
{
    ...
    if (!empty($page_data)) {
		...
		fn_attach_image_pairs('page_image_main', 'page_image', $page_id, $lang_code);
    }
...
function fn_get_page_data($page_id, $lang_code = CART_LANGUAGE, $preview = false, $area = AREA)
{
    ...
    if (empty($cache[$page_id])) {
		...
		$page_data['main_pair'] = fn_get_image_pairs($page_id, 'page_image', 'M', true, true, $lang_code);
...
function fn_get_child_pages($parent_id){
	$ids = db_get_fields('SELECT page_id FROM ?:pages WHERE parent_id = ?i', $parent_id);
	$pages = array();
	foreach($ids as $id){
		$page = fn_get_page_data($id);
		$pages []= $page;
	}
	return $pages;
}